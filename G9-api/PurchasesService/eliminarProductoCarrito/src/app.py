...
@app.route('/buy/deleteProductCart', methods=['DELETE'])
def eliminar_producto_carrito():
    form = validators.EliminarProductoCarrito(request.form)
    ...
    try:
        ...
        try:
            ...
            cur.execute("select * from detalle_carrito d inner join Producto p on d.id_producto = p.id_producto inner join Categoria c on c.id_categoria = p.id_categoria where p.id_producto = "+str(form.idProducto.data)+" and d.id_usuario = "+cui+"and d.estado = 'P';")
            if cur.rowcount > 0:
                sql = "UPDATE detalle_carrito SET estado = 'C' WHERE estado = 'P' AND id_usuario = %s AND id_producto = %s;"
                values = (cui,form.idProducto.data)
                cur.execute(sql, values)
                conn.commit()
            else:
                return jsonify({'estatus': 502, 'error': 'Error en parametros de entrada', 'mensaje': 'Error el id del producto no existe en el carrito de comrpas'}), 502
        ...
        respuesta = {
            "estatus": 200,
            "mensaje": "El producto del carrito de compras ha sido eliminado exitosamente"
        }
